...
function createTask() {
  ...
  okButtonEl.addEventListener("click", () => {
    ...
    if (createTaskState.step === 0) {
      ...
      if (createTaskState.user_id !== "")
        payload.user_id = createTaskState.user_id;
...
function createTaskElement(task) {
  ...
  const assignedTo = task.user
    ? `${task.user.first_name} ${task.user.last_name} (${task.user.username})`
    : "Unassigned";

  user.textContent = `Assigned to: ${assignedTo}`;
